...
为了更加正式地描述监督学习问题，我们的目标是，给定一个训练集，学习得到一个函数$h:X\to Y$，$h(x)$是相对应$y$值的“良好”预测器。由于历史原因，这个函数$h$被称作“**假设**”。如下图所示，，这个过程大概是这样的:<center>![](img\in-post\machine_learning\coursera_stanford_andrew_ng\week1\model_representation.png)</center>
...
这个函数也叫做“平方差函数”，或者叫做“平均方差”。平均值取($\frac{1}{2}$)是为了梯度下降计算的方便，平方的导数可以与$\frac{1}{2}$抵消掉。下图简要说明了代价函数做了什么:<center>![](img\in-post\machine_learning\coursera_stanford_andrew_ng\week1\what_cost_function_does.png)</center>
